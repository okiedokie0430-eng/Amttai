...
class _HomeScreenState extends State<HomeScreen> {
  ...
  bool _prefetchedFeaturedImages = false;
  ...
  void _maybePrefetchFeaturedImages(List<Recipe> recipes) {
    if (_prefetchedFeaturedImages || recipes.isEmpty) {
      return;
    }

    _prefetchedFeaturedImages = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) {
        return;
      }
      ...
      for (final recipe in recipes.take(5)) {
        ...
        precacheImage(
          ResizeImage(CachedNetworkImageProvider(imageUrl), height: 720),
          context,
        );
      }
    });
